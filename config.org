...
|---------------+---------------------------------|
| General       |                                 |
| =C-c C-e=     | instert environment             |
| =C-c C-s=     | insrt section                   |
| =C-c C-f=     | font changes                    |
| =C-c C-o=     | autohide section                |
| =`=           | to get suggestion in math mode  |
| =C-c C-q C-e= | to indent environemnt           |
|---------------+---------------------------------|
| Reftex        | =M-x reftex-info=               |
| =C-c (=       | create a citation               |
| =C-c )=       | refferences a citation          |
| =C-c ==       | create content list to navigate |
| =C-c [=       | citation                        |
|---------------+---------------------------------|
| Cdflatex      |                                 |
| ="C-c {"=     | list the availble templates     |
|---------------+---------------------------------|
| Compling      |                                 |
...
#+BEGIN_SRC emacs-lisp 
 ...
 #+END_SRC
** pdf synchronisation
must specify =EMACS_PATH\=~/.emacs.d= in =.profile=
...
*** command to generate file using latexmk
1- if files have already been generated into the output directory, copy them
2- a pdf (=-pdf=) file is generated and automatically checked (=-pvc=) for changes using the supplied file (=%s=)
  ;;                              '("gen_ilya" "mv _output/%s.* . 2> /dev/null; latexmk -pdf -pvc %s" TeX-run-TeX nil t
#+BEGIN_SRC emacs-lisp
...
1- remove auto directory and copy and files with this master file name into the output directoty
#+BEGIN_SRC emacs-lisp 
  (add-hook 'LaTeX-mode-hook (lambda ()
                               (push
                               '("jew_ilya" "/Users/CCCP/.emacs.d/ilya_scripts/jew_engine.sh %s" TeX-run-TeX nil t
                               ;; '("jew_ilya" "rm -f -r auto; mkdir -p _output; mv %s.* _output; mv _output/%s.pdf _output/%s.tex ." TeX-run-TeX nil t
                                  :help "clean directory from nigger crap")
                                TeX-command-list)))

  ;;'("jew_ilya" "latexmk -c %s && rm -r auto" TeX-run-TeX nil t
 #+END_SRC
*** Program for when view (="C-c C-v"=) is run
...
#+BEGIN_SRC emacs-lisp
  ;; -b puts a reading bar up
  (setq TeX-view-program-list
        '(("SkimViewer" "/Users/CCCP/.emacs.d/ilya_scripts/search_engine.sh %s %n %o %b")))
  ;; %s %n %o %a")))
        ;;      '(("SkimViewer" "mv _output/%s.synctex.gz . ; /Applications/Skim.app/Contents/SharedSupport/displayline -b -g %n %o %b")))
