...
- [[https://github.com/rust-lang/rls][rls]] (rust language server), must be also installed with =rustup= (see [[*External setup][External setup]])
- cargo (for compling project)
- [[https://github.com/brotzeit/rustic][rustic]] mode does compilation, checking and documentation. Can activate later on
|-------------+---------------------------------+-------|
...
|-------------+---------------------------------+-------|
** External setup
Actual rust in installed with (don't forget to add =source ~/.cargo/env= to =.profile=)
...
** todo lists
#+BEGIN_SRC emacs-lisp 
  (setq org-todo-keywords '((sequence "TODO(t)" "BLOAT(B)" "BRYAN(z)" "PHD(p)" "DREAMS-AI(a)" "HORSE(h)" "SA36(3)" "WORKFORCE(w)" "SCHOOLS(b)" "TUTORING(l)" "CURRENT(c)" "SKYPE(s)" "|" "DOMINATED(d)")))
...
#+BEGIN_SRC emacs-lisp 
  ...
  (defun inkscape-generate (base-directory)
    ...
    (let* (;; a - generate filename
      ...
      (insert (concat "[[file:images_inkscape/" file-name ".png][" file-name "]]"))
      ))
