...
#+BEGIN_SRC emacs-lisp 
  (use-package auctex
    ...
    :ensure t
    :config(
            (setq TeX-auto-save t) ;; document parsing enabled
            (setq TeX-parse-self t)
            (setq-default Tex-master nil) ;; every time a file is opened, specify the master file that inputs it

            (add-hook 'LaTeX-mode-hook 'flyspell-mode) ;;spell checking
            (add-hook 'LaTeX-mode-hook 'LaTeX-math-mode) ;;show math

            (add-hook 'LaTeX-mode-hook 'turn-on-reftex) ;;refferencing
            (setq reftex-plug-into-AUCTeX t)
            ))

  ;; to install predictive, copy the github repository 'predictive' into the emacs.d folder
  ...
  ;;			     (push
  ;;			      '("Make" "latexmk -outdir=/tmp "))))

  ;;even better autocompletion
  ;;(use-package cdlatex
  ;;  :ensure t)
  ;;(add-hook 'Latex-mode-hook 'turn-on-cd)
  ;;
  ;;
  ;;
...
* Terminal
** shell mode shortcut in org mode
#+BEGIN_SRC emacs-lisp
  (add-to-list 'org-structure-template-alist
	       '("sh" "#+BEGIN_SRC sh\n?\n#+END_SRC"))
#+END_SRC
