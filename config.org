...
#+BEGIN_SRC emacs-lisp
  ...
  "
    ...
    ("l"   (ilya-pyenv-activate "~/creamy_seas/sync_files/python_vi/linux"))
...
** todo lists
#+BEGIN_SRC emacs-lisp 
  (setq org-todo-keywords '((sequence "TODO(t)" "BLOAT(B)" "BRYAN(b)" "PHD(p)" "DREAMS-AI(a)" "SCHOOLS(s)" "TUTORING(l)" "CURRENT(c)" "|" "DOMINATED(d)")))
...
#+BEGIN_SRC emacs-lisp 
  (setq org-agenda-files (list "~/creamy_seas/antlers.org"
                               "~/creamy_seas/2am/east-data-company.org"))
...
#+BEGIN_SRC emacs-lisp 
  ...
  (defun inkscape-generate (base-directory)
    ...
    (let* (;; a - generate filename
      ...
      (minibuffer-message (concat inkscape-generate-location " " file-name-full))
