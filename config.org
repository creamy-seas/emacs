...
- a process name is generated based off the name of the master file
- the actual command calls the =pdf_engine= script in the emacs.d directory
- expansion if performed to change =%s= to the master file name
#+BEGIN_SRC emacs-lisp 
  (defun ilya_gen-key ()
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun ilya_jew-key()
   ...
   (let (
     ...
     ;;2) delete the buffer the window was in
     (kill-buffer "卍 Compiling Jews 卍")

     ;;3) move the files in to the output directory
     (ignore-errors (TeX-run-TeX "jew_process" (TeX-command-expand "/Users/CCCP/.emacs.d/ilya_scripts/jew_engine.sh %s" 'TeX-master-file TeX-expand-list) (TeX-master-file)))
