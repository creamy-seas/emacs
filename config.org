...
- fireUpScript is a function that creates a string from the emacs home directory
#+BEGIN_SRC emacs-lisp 
  ...
  (defun fireUpScript(scriptName)
    (setq path (exec-path-from-shell-copy-env "EMACS_HOME"))
    (concat path "/" scriptName)
    )
 #+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun jew_key()
    (interactive)
    (minibuffer-message (TeX-master-file))
    ...
    (TeX-run-TeX "jew_process"
                 (TeX-command-expand "/Users/CCCP/.emacs.d/ilya_scripts/jew_engine.sh %s" 'TeX-master-file)
                 (TeX-master-file))
    ;;4) close this window
  ...
  (add-hook 'LaTeX-mode-hook (lambda ()
                               ...
                               (local-set-key (kbd "C-c C-j") (function jew_key))
                               (global-set-key (kbd "C-c C-j") (function jew_key))))
...
#+BEGIN_SRC emacs-lisp 
  ...
  ;;  (if (not (eq (get-process "gen_ilya") nil))
    (if (eq (get-buffer "卍 Compiling Jews 卍") nil)
        ...
        (progn
          (minibuffer-message "Creating compile window")
          (rename-buffer "卍 Compiling Jews 卍")
          )
      ;;b) if this is called after a compilation, delete the original compilation window and rename this one
      (progn
  ;;      (kill-buffer (get-buffer "卍 Compiling Jews 卍"))
  ;;      (rename-buffer "卍 ====> Gassing (((Jews))) 卍")
  ;;      (minibuffer-message "Deleting compile window")
        )
    ))
...
#+BEGIN_SRC emacs-lisp
  (use-package yasnippet
    ...
    (yas-global-mode)
    :config
    (add-to-list 'yas-snippet-dirs (fireUpScript "snippets/snippet-mode"))
