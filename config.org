...
|                           |                                                                            |
- and important variable is =TeX-expand-list-builtin= which defines variables such as =%s= =%t=
- using the function =TeX-command-expand= with variables ="COMMAND TO CALL"= =masterFile= and =TeX-expand-list-builtin=
*** command to generate file
#+BEGIN_SRC emacs-lisp
  (add-hook 'LaTeX-mode-hook (lambda ()
                               (setq argument-list
                                     `(TeX-run-TeX nil t :help "run latemk on the master file"))
                               (setq command-script
                                     (ilya_fire-up-from-emacs
                                                   "ilya_scripts/pdf_engine.sh %s"))
                               (setq process-name
                                     (ilya_generate-process-name "genPDF"))

                               (push command-script argument-list)
                               (push process-name argument-list)
                               (push argument-list
                                     ;;                              `("gen_ilya" "/Users/CCCP/.emacs.d/ilya_scripts/pdf_engine.sh %s" TeX-run-TeX nil t
                                     TeX-command-list)))

  (add-hook 'LaTeX-mode-hook
            (lambda () (setq TeX-command-default "gen_ilya")))
 #+END_SRC
*** key command generate file
#+BEGIN_SRC emacs-lisp 
  (defun ilya_gen-key ()
    "Command binded to C-c C-g will generate pdf with latexmk"
    (interactive)
    (let (;;a) variable definition
          (process-name (format "%s" (ilya_generate-process-name "genPDF")))
          (command-script (ilya_fire-up-from-emacs "ilya_scripts/pdf_engine.sh %s"))
          (expanded-command-script
           (TeX-command-expand command-script 'TeX-master-file TeX-expand-list)))

      ;;b) programming
      (TeX-run-TeX process-name expanded-command-script (TeX-master-file))))

  (add-hook 'LaTeX-mode-hook (lambda ()
                               (local-set-key (kbd "C-c C-g") (function ilya_gen-key))))
 #+END_SRC
*** clean the directory of processed files
#+BEGIN_SRC emacs-lisp 
  (add-hook 'LaTeX-mode-hook (lambda ()
                               (push
                                '("jew_ilya" "/Users/CCCP/.emacs.d/ilya_scripts/jew_engine.sh %s" TeX-run-TeX nil t
                                  :help "clean directory from nigger crap")
                                TeX-command-list)))
 #+END_SRC
**** this is the =C-c C-j= binding to the same command
1 - stop and kill any pdf compilation process called ="gen_ilya"= without confirmation
2 - delete the buffer that was running the pdf
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun ilya_generate-process-name (process_name)
    "Generates a string of the form [processName_masterFile]"
    (concat process_name "_" (ilya_get-master-file-name)))

  (defun test()
    (interactive)
    (setq command-script (format "%S" (ilya_fire-up-from-emacs "ilya_scripts/pdf_engine.sh %s")))
    (setq process-name (format "%S" (ilya_generate-process-name "genPDF")))
    (setq other-arguments `(TeX-run-TeX nil t :help "run latemk on the master file"))
    (push command-script other-arguments)
    (push process-name other-arguments) 
    (minibuffer-message other-arguments))


  (defun ilya_jew-key()
    (interactive)
    (ignore-errors
      ;;  (minibuffer-message (concat "卍 Exterminating " (TeX-master-file) " 卍"))

      (minibuffer-message other-arguments)
      ;;1) delete "ilya_gen" processes (pdf generation) processes
    (ignore-errors (set-process-query-on-exit-flag (get-process "gen_ilya") nil))
    (ignore-errors (delete-process (get-process "gen_ilya")))
    ;;2) run the process to delete the buffer window
    (ignore-errors (kill-buffer "卍 Compiling Jews 卍"))
    ;;3) move the files in directory

    ;;  (minibuffer-message (TeX-command-expand "/Users %s" 'Tex-master-file TeX-expand-list))
    (TeX-run-TeX "jew_process" (TeX-command-expand "/Users/CCCP/.emacs.d/ilya_scripts/jew_engine.sh %s" 'TeX-master-file TeX-expand-list) (TeX-master-file)))
    )
    ;;4) close this window
  ;;  (ignore-errors (kill-buffer (get-buffer "卍 Compiling Jews 卍")))
    ;;(minibuffer-message "===> 卍 extermination complete 卍")
      ;;3) kill the buffers
  ;;   (ignore-errors (kill-buffer (get-buffer "卍 Compiling Jews 卍")))
  ;;   (ignore-errors (kill-buffer (get-buffer "卍 ====> Gassing (((Jews))) 卍"))))
     ;;    (minibuffer-message "====> jew have been cleared - 卍 heil! 卍"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (server-start)
 #+END_SRC
*** compilation window resize
#+BEGIN_SRC emacs-lisp 
  (defun LaTeX-compilation-buffer-size ()
    "Resize the latex compilation buffer when it launches because it is seriosuly bloat"
    ;;1) check if a compilation window is running 
  ;;  (if (not (eq (get-process "gen_ilya") nil))
    (if (eq (get-buffer "卍 Compiling Jews 卍") nil)
        ;;a) if the window does not exist, create a new compilation window
        (progn
          (ignore-errors (minibuffer-message "Renaming compile window to \"卍 Compiling Jews 卍\""))
          (ignore-errors (rename-buffer "卍 Compiling Jews 卍"))
          )
      ;;b) if this is called after a compilation, delete the original compilation window and rename this one
        )
    )

  (add-hook 'comint-mode-hook (function LaTeX-compilation-buffer-size))
 #+END_SRC
