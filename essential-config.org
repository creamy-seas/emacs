#+TITLE: Essential config that makes life easier
#+STARTUP: overview
...
#+BEGIN_SRC emacs-lisp
  (defun my/config-file-path-evaluate (path-from-config-folder)
    "A shorthand for expanding the name of a file into the config
  directory with the full location"
      ...
      (when (member current-file my/config-files)
        (let ((prog-mode-hook nil)		;; Avoid running hooks when tangling.
          ...
          (let ((el-compiled-file (my/config-file-path-evaluate el-file))
            ...
            (message (concat "Compiled" el-compiled-file))
            (byte-compile-file el-emacs-file))))))
